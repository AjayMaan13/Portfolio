...
document.addEventListener('DOMContentLoaded', () => {
    // Tour steps - each defines a target element and content
    const tourSteps = [
        {
            target: '.hero-content',
...
function showTourStep(stepIndex) {
    ...
    // Position the popup
    positionPopup(tourPopup, targetRect, step.position);
...
function positionPopup(popup, targetRect, position) {
    ...
    switch (position) {
        case 'top':
            popup.style.bottom = `${window.innerHeight - targetRect.top + 10}px`;
            popup.style.left = `${targetRect.left + targetRect.width / 2 - popupRect.width / 2}px`;
            break;
        case 'right':
            popup.style.left = `${targetRect.right + 10}px`;
            popup.style.top = `${targetRect.top + targetRect.height / 2 - popupRect.height / 2}px`;
            break;
        case 'bottom':
            ...
            break;
        case 'left':
            popup.style.right = `${window.innerWidth - targetRect.left + 10}px`;
            popup.style.top = `${targetRect.top + targetRect.height / 2 - popupRect.height / 2}px`;
            break;
    }
    ...
    } else if (updatedPopupRect.right > window.innerWidth - 10) {
